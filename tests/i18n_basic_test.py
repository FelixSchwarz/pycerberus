...
class MessagesFromBuiltInValidatorsAreTranslatedTest(ValidationTest):
    ...
    def test_error_messages_are_translated(self):
        self.assert_equals('Please enter a number.', self.get_error('foo', locale='en').error().msg)
        self.assert_equals('Bitte geben Sie eine Zahl ein.', self.get_error('foo', locale='de').error().msg)
    
    def test_variable_parts_are_added_after_translation(self):
        expected_message = u'(String erwartet, "list" erhalten)'
        translated_message = self.get_error([], locale='de').error().msg
